...
/obj/machinery/bodyscanner/MouseDrop_T(mob/living/carbon/O, mob/user as mob)
    ...
    if(user.incapacitated())
        return 0 // User should not be doing things.

    if(O.anchored)
        return 0 // Mob is anchored.

    if((get_dist(user, src) > 1) || (get_dist(user, O) > 1))
        return 0 // Does not use adjacent() to allow non-cardinal interaction.
    ...
    if(computer_deconstruction_screwdriver(user, I))
        ...
        if(P.connectable)
            if(istype(P.connectable, /obj/machinery/bodyscanner))
                ...
                scanner = C
                C.console = src
                to_chat(user, "<span class='warning'>You link [src] to [P.connectable]!</span>")
        else
            to_chat(user, "<span class='warning'>You store [src] in [P]'s buffer!</span>")
            P.connectable = src
...
/obj/machinery/body_scanconsole/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
    ...
    if(scanner)
        ...
        if(scanner.occupant && ishuman(scanner.occupant))
            ...
            if(H.reagents && H.reagents.reagent_list.len >= 1)
            ...
            if(H.ingested && H.ingested.reagent_list.len >= 1)
            ...
            for(var/obj/item/organ/external/E in H.organs)
                ...
                for(var/obj/I in E.implants)
                    ...
                    implantData += list(implantSubData)
