...
/proc/Get_Holiday()
    ...
    // Holidays use text-keyed associative entries:
    // Holiday["Holiday Name"] = "Holiday description"
    switch(MM)
        if(1)
            switch(DD)
                if(1)
        ...
        if(4)
            switch(DD)
                ...
                if(22)
                    Holiday["Earth Day"] = "A holiday of environmentalism, that originated on its namesake, Earth."
        ...
        if(6)
            switch(DD)
                ...
                if(20)
                    ...
                    especially those who are members of the armed forces and the emergency services, or have been civil servants in the past."
...
/proc/Holiday_Game_Start()
    if(Holiday.len != 0)
        ...
        var/holidays_string = english_list(
            holidays,
        ...
        if(("Christmas Eve" in Holiday) || ("Christmas" in Holiday))
            Christmas_Game_Start()

    return


// Nested in the random events loop. Triggered every 2 minutes.
/proc/Holiday_Random_Event()
    if(isemptylist(Holiday))
        return 0

    if("End of the World" in Holiday)
        if(prob(eventchance))
            GameOver()

    if(("Christmas" in Holiday) || ("Christmas Eve" in Holiday))
        if(prob(eventchance))
            ChristmasEvent()
